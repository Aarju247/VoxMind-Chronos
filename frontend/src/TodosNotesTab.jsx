...
const TodosNotesTab = () => {
  const userId = localStorage.getItem("gcal_user_id") || "default"
  ...
  const loadTodos = useCallback(async () => {
    ...
    try {
      const r = await fetch(`${BASE}/todos?user_id=${encodeURIComponent(userId)}`)
  ...
  const loadNotes = useCallback(async () => {
    ...
    try {
      const r = await fetch(`${BASE}/notes?user_id=${encodeURIComponent(userId)}`)
      const d = await r.json()
      setNotes(d.notes || [])
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      const r = await fetch(`${BASE}/local-events?user_id=${encodeURIComponent(userId)}`)
      const d = await r.json()
      setEvents(d.events || [])
  ...
  const addTodo = async () => {
    ...
    try {
      const r = await fetch(`${BASE}/todos`, {
        ...
        body: JSON.stringify({ task: newTask.trim(), user_id: userId }),
      })
      const d = await r.json()
  ...
  const toggleTodo = async (index) => {
    try {
      await fetch(`${BASE}/todos/${index}/complete?user_id=${encodeURIComponent(userId)}`, { method: "PATCH" })
  ...
  const deleteTodo = async (index) => {
    try {
      const r = await fetch(`${BASE}/todos/${index}?user_id=${encodeURIComponent(userId)}`, { method: "DELETE" })
      const d = await r.json()
      if (d.error) { flash(setTodoMsg, `❌ ${d.error}`); return }
      setTodos(d.todos.map((t, i) => ({ ...t, index: i })))
  ...
  const addNote = async () => {
    ...
    try {
      const r = await fetch(`${BASE}/notes`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ content: newNote.trim(), user_id: userId }),
      })
      const d = await r.json()
  ...
  const deleteNote = async (index) => {
    try {
      const r = await fetch(`${BASE}/notes/${index}?user_id=${encodeURIComponent(userId)}`, { method: "DELETE" })
      const d = await r.json()
      if (d.error) { flash(setNoteMsg, `❌ ${d.error}`); return }
      setNotes(d.notes.map((n, i) => ({ ...n, index: i })))
  ...
  const deleteEvent = async (index) => {
    try {
      const r = await fetch(`${BASE}/local-events/${index}?user_id=${encodeURIComponent(userId)}`, { method: "DELETE" })
      const d = await r.json()
  ...
  const pillStyle = (active, color) => ({
    ...
    color: active ? color : "rgba(255,255,255,0.4)", transition: "all 0.18s",
  })

  const delBtn = {
    background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.25)",
    borderRadius: "7px", padding: "5px 6px", cursor: "pointer", color: "#fca5a5",
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "18px", display: "flex", flexDirection: "column", gap: "14px" }}>
      ...
      <div style={{ display: "flex", gap: "7px", flexWrap: "wrap" }}>
        <button onClick={() => setSection("todos")}  style={pillStyle(section === "todos",  "#a78bfa")}>✅ Todos ({todos.length})</button>
        <button onClick={() => setSection("notes")}  style={pillStyle(section === "notes",  "#67e8f9")}>📝 Notes ({notes.length})</button>
        <button onClick={() => setSection("events")} style={pillStyle(section === "events", "#fbbf24")}>📅 Chat Events ({events.length})</button>
      </div>

      {section === "todos" && (
        <div style={{ ...glass.card, padding: "16px" }}>
          <SectionHead icon={<CheckSquare size={15} />} title="Todos" count={`${pendingCount} pending · ${completedCount} done`} color="#a78bfa" onRefresh={loadTodos} />
          <Flash msg={todoMsg} />
          <div style={{ display: "flex", gap: "8px", marginBottom: "14px" }}>
            <input value={newTask} onChange={e => setNewTask(e.target.value)} onKeyDown={e => e.key === "Enter" && addTodo()} placeholder="Add a new task…" style={inp} />
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:"6px" }}>
              {todos.map(todo => (
                <div key={todo.index} style={{ display:"flex",alignItems:"center",gap:"10px",padding:"9px 12px",background:todo.completed?"rgba(52,211,153,0.05)":"rgba(167,139,250,0.07)",border:`1px solid ${todo.completed?"rgba(52,211,153,0.18)":"rgba(167,139,250,0.15)"}`,borderRadius:"10px" }}>
                  <button onClick={() => toggleTodo(todo.index)} style={{ width:"20px",height:"20px",borderRadius:"6px",border:`2px solid ${todo.completed?"#34d399":"rgba(167,139,250,0.5)"}`,background:todo.completed?"rgba(52,211,153,0.25)":"transparent",cursor:"pointer",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"11px" }}>
                    {todo.completed ? "✓" : ""}
                  </button>
                  <span style={{ flex:1,fontSize:"13px",color:todo.completed?"rgba(255,255,255,0.35)":"rgba(255,255,255,0.85)",textDecoration:todo.completed?"line-through":"none" }}>{todo.task}</span>
                  ...
                  <button onClick={() => deleteTodo(todo.index)} style={delBtn}><Trash2 size={12} /></button>
                </div>
              ))}
            </div>
          )}
      ...
      {section === "notes" && (
        <div style={{ ...glass.card, padding: "16px" }}>
          <SectionHead icon={<FileText size={15} />} title="Notes" count={notes.length} color="#67e8f9" onRefresh={loadNotes} />
          <Flash msg={noteMsg} />
          <div style={{ display:"flex",gap:"8px",marginBottom:"14px" }}>
            <textarea value={newNote} onChange={e => setNewNote(e.target.value)} placeholder="Jot down a note…" rows={2} style={{ ...inp,resize:"vertical",fontFamily:"inherit" }} />
          ...
          {notesLoading ? (
            ...
            <p style={{ fontSize:"13px",color:"rgba(255,255,255,0.3)",margin:0 }}>No notes yet.</p>
          ) : (
            <div style={{ display:"flex",flexDirection:"column",gap:"7px" }}>
              {notes.map(note => (
                <div key={note.index} style={{ display:"flex",alignItems:"flex-start",gap:"10px",padding:"11px 13px",background:"rgba(103,232,249,0.05)",border:"1px solid rgba(103,232,249,0.14)",borderRadius:"10px" }}>
                  <p style={{ flex:1,fontSize:"13px",color:"rgba(255,255,255,0.82)",margin:0,lineHeight:1.6,whiteSpace:"pre-wrap" }}>{note.content}</p>
                  <button onClick={() => deleteNote(note.index)} style={delBtn}><Trash2 size={12} /></button>
                </div>
              ))}
            </div>
          )}
      ...
      {section === "events" && (
        <div style={{ ...glass.card, padding: "16px" }}>
          <SectionHead icon={<Calendar size={15} />} title="Chat-Scheduled Events" count={events.length} color="#fbbf24" onRefresh={loadEvents} />
          <div style={{ padding:"9px 13px",background:"rgba(251,191,36,0.07)",border:"1px solid rgba(251,191,36,0.2)",borderRadius:"10px",fontSize:"12px",color:"rgba(255,255,255,0.5)",marginBottom:"14px",lineHeight:1.6 }}>
            Events created via chat like <span style={{ color:"#fbbf24" }}>"Schedule team meeting tomorrow at 3pm"</span>. For Google Calendar, use the <span style={{ color:"#a78bfa" }}>📅 Calendar tab</span>.
          </div>
          <Flash msg={eventMsg} />
          {eventsLoading ? (
            <div style={{ display:"flex",gap:"4px",padding:"10px 0" }}>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:"7px" }}>
              {events.map(evt => (
                <div key={evt.index} style={{ display:"flex",alignItems:"flex-start",gap:"10px",padding:"11px 13px",background:"rgba(251,191,36,0.06)",border:"1px solid rgba(251,191,36,0.18)",borderRadius:"10px" }}>
                  <div style={{ flex:1 }}>
                    <p style={{ fontSize:"13px",fontWeight:600,color:"#f0f0ff",margin:"0 0 3px" }}>📅 {evt.title}</p>
                    {evt.datetime && (
                      <p style={{ fontSize:"11px",color:"#fbbf24",margin:0 }}>
                        {(() => { try { return new Date(evt.datetime).toLocaleString([],{weekday:"short",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}) } catch { return evt.datetime } })()}
                      </p>
                    )}
                  </div>
                  <button onClick={() => deleteEvent(evt.index)} style={delBtn}><Trash2 size={12} /></button>
                </div>
              ))}
            </div>
          )}
