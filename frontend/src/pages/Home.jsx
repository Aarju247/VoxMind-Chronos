...
const Home = () => {
  ...
  const refreshDashboard = async () => {
    try {
      ...
      const d      = await fetch(`${BASE}/dashboard${qs}`).then(r => r.json())
      setDashboard(d)
  ...
  try {
    const res  = await API.get(`/ask?question=${encodeURIComponent(q)}&user_id=${encodeURIComponent(userId)}`)
    ...
    if (q.toLowerCase().includes("schedule") && q.toLowerCase().includes("tomorrow")) {
  ...
  const generateSummary = async () => {
    ...
    try {
      const r = await fetch(`${BASE}/pdf-summary?filename=${encodeURIComponent(pdfName)}`, { method: "POST" })
  ...
  const deletePdf = async (filename) => {
    try {
      await fetch(`${BASE}/pdf-delete?filename=${encodeURIComponent(filename)}`, { method: "DELETE" })
 ...
 const loadMemories = async () => {
  ...
  const userId = localStorage.getItem("gcal_user_id") || "default"
  try {
    const r    = await fetch(`${BASE}/memories?user_id=${encodeURIComponent(userId)}`)
...
const saveFact = async () => {
  ...
  const userId = localStorage.getItem("gcal_user_id") || "default"
  try {
    const r    = await fetch(`${BASE}/memories/save`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fact: newFact.trim(), user_id: userId }) })
    const data = await r.json()
  ...
  const userId = localStorage.getItem("gcal_user_id") || "default"
  try {
    await fetch(`${BASE}/memories/${id}?user_id=${encodeURIComponent(userId)}`, { method: "DELETE" })
  ...
  const userId = localStorage.getItem("gcal_user_id") || "default"
  try {
    const r    = await fetch(`${BASE}/memories/search`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query: memSearchQuery, top_k: 5, user_id: userId }) })
    const data = await r.json()
  ...
  const userId = localStorage.getItem("gcal_user_id") || "default"
  try {
    const r = await fetch(`${BASE}/ask?question=analyze my productivity&user_id=${encodeURIComponent(userId)}`)
...
//   `/ask?question=${encodeURIComponent(q)}&user_id=${encodeURIComponent(userId)}`
// )
//       const text = res.data.response
//       setMessages(p => [...p, { role: "assistant", text }])
//       speakResponse(text)
...
//     setPdfLoading(true); setRagStep("🧠 Summarizing full document…")
//     try {
//       const r = await fetch(`${BASE}/pdf-summary?filename=${encodeURIComponent(pdfName)}&user_id=${encodeURIComponent(userId)}`, { method: "POST" })
//       const d = await r.json()
//       setPdfSummary(d.summary || d.error)
//     } catch { setPdfSummary("Failed to generate summary.") }
//     finally { setPdfLoading(false); setRagStep("") }
...
//         method:  "POST",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({ filename: pdfName, question: pdfQuestion, user_id: userId }),
//       })
//       const data = await res.json()
//       setPdfAnswer(data.answer || data.error)
//       setPdfChunks(data.chunks || [])
...
//   const deletePdf = async (filename) => {
//     try {
//       await fetch(`${BASE}/pdf-delete?filename=${encodeURIComponent(filename)}&user_id=${encodeURIComponent(userId)}`, { method: "DELETE" })
//       setPdfList(p => p.filter(f => f !== filename))
//       if (pdfName === filename) { setPdfName(""); setIngestStats(null); setPdfAnswer(""); setPdfChunks([]) }
//     } catch (e) { console.error(e) }
//   }
...
//     try {
//       const r = await fetch(
//   `${BASE}/ask?question=analyze my productivity&user_id=${encodeURIComponent(userId)}`
// )
//       setInsights((await r.json()).response)
//     } catch (e) { console.error(e) }
//     finally { setLoadingInsights(false) }
