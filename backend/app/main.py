...
@app.get("/ask")
async def ask_ai(question: str, user_id: str = Query(default="default")):
    try:
        lower_question = question.lower()

        if "remember that" in lower_question:
            fact = re.sub(r"remember that\s*", "", question, flags=re.IGNORECASE).strip()
            save_memory(fact, user_id=user_id)
            return {"question": question, "response": f"Got it! I'll remember: {fact}"}
        ...
        if "search my memories" in lower_question or "recall" in lower_question:
            query = lower_question.replace("search my memories about", "").replace("search my memories", "").replace("recall", "").strip()
            results = search_memories(query, top_k=5, user_id=user_id)
            if not results:
                return {"response": f"No memories found related to '{query}'."}
            result_text = "\n".join([f"• {r['fact']} (relevance: {r['score']})" for r in results])
            return {"response": f"Most relevant memories for '{query}':\n\n{result_text}"}
        ...
        if "add" in lower_question and "todo" in lower_question:
            task = lower_question.replace("add", "").replace("to my todo list", "").replace("todo", "").strip()
            save_todo({"task": task, "completed": False}, user_id=user_id)
            return {"response": f"Added '{task}' to your todo list."}

        if "show my todo list" in lower_question or "show my todos" in lower_question:
            todos = get_all_todos(user_id=user_id)
            if not todos:
                return {"response": "Your todo list is empty."}
            todo_text = "\n".join([f"{'✅' if t['completed'] else '⬜'} {t['task']}" for t in todos])
            return {"response": f"Here are your tasks:\n\n{todo_text}"}
        ...
        if "mark" in lower_question and "completed" in lower_question:
            task_name = lower_question.replace("mark", "").replace("as completed", "").strip()
            complete_todo(task_name, user_id=user_id)
            return {"response": f"Marked '{task_name}' as completed."}
        ...
        if "schedule" in lower_question and "tomorrow" in lower_question:
            ...
            if match:
                ...
                if meridian == "am" and hour == 12: hour  = 0
                event_time = (datetime.now() + timedelta(days=1)).replace(hour=hour, minute=0, second=0, microsecond=0)
                save_event({"title": title, "datetime": event_time.isoformat()}, user_id=user_id)
                return {"response": f"Scheduled: {title} tomorrow at {match.group(2)} {meridian}"}
            return {"response": "Try: 'Schedule <title> tomorrow at <H> am/pm'"}

        if "show events" in lower_question or "show all events" in lower_question:
            events = get_all_events(user_id=user_id)
            if not events:
                return {"response": "No events scheduled."}
            return {"response": "\n".join([f"📅 {e['title']} — {e['datetime']}" for e in events])}
        ...
        if "schedule today" in lower_question or "agenda today" in lower_question:
            today = datetime.now().date()
            today_events = [e for e in get_all_events(user_id=user_id) if datetime.fromisoformat(e["datetime"]).date() == today]
            if not today_events:
                return {"response": "No events today."}
            return {"response": "Today's agenda:\n\n" + "\n".join([f"📅 {e['title']}" for e in today_events])}
        ...
        if "save note" in lower_question:
            note_text = question.replace("Save note:", "").replace("save note", "").strip()
            save_note({"content": note_text}, user_id=user_id)
            return {"response": "Note saved."}

        if "search notes" in lower_question:
            term = lower_question.replace("search notes about", "").replace("search notes", "").strip()
            matches = [n["content"] for n in get_all_notes(user_id=user_id) if term in n["content"].lower()]
            if not matches:
                return {"response": f"No notes found about '{term}'."}
            return {"response": "\n".join([f"• {n}" for n in matches])}
        ...
        if "add to my calendar" in lower_question or "schedule on my calendar" in lower_question:
            return {"response": "To create a calendar event, use the Calendar tab → '+ New Event'."}

        # ── GENERIC AI CHAT with semantic memory context ──
        conversation_history.append({"role": "user", "content": question})
        relevant_memories = search_memories(question, top_k=5, user_id=user_id)
        memory_context    = "\n".join([f"- {m['fact']}" for m in relevant_memories])
        enhanced_history  = [
            {"role": "system", "content": f"You are VoxMind Chronos, an AI second brain.\n\nRelevant user memories:\n{memory_context or 'None yet'}"}
        ]
...
@app.get("/todos")
async def list_todos(user_id: str = Query(default="default")):
    todos = get_all_todos(user_id=user_id)
    return {
        "todos": [{"index": i, **t} for i, t in enumerate(todos)],
        "count": len(todos),
...
async def add_todo(data: dict):
    ...
    if not task:
        return {"error": "task cannot be empty"}
    save_todo({"task": task, "completed": False}, user_id=user_id)
    return {"message": f"Todo added: {task}", "todos": get_all_todos(user_id=user_id)}

@app.patch("/todos/{index}/complete")
async def api_complete_todo(index: int, user_id: str = Query(default="default")):
    ok = toggle_todo(index, user_id=user_id)
    if not ok:
        return {"error": f"No todo at index {index}"}
    todos = get_all_todos(user_id=user_id)
    return {"message": "Toggled", "todo": {**todos[index], "index": index}}
...
@app.delete("/todos/{index}")
async def api_delete_todo(index: int, user_id: str = Query(default="default")):
    ok = delete_todo(index, user_id=user_id)
    if not ok:
        return {"error": f"No todo at index {index}"}
    return {"message": "Deleted", "todos": get_all_todos(user_id=user_id)}
...
@app.get("/notes")
async def list_notes(user_id: str = Query(default="default")):
    notes = get_all_notes(user_id=user_id)
    return {
        "notes": [{"index": i, **n} for i, n in enumerate(notes)],
        "count": len(notes),
...
async def add_note(data: dict):
    ...
    if not content:
        return {"error": "content cannot be empty"}
    save_note({"content": content}, user_id=user_id)
    return {"message": "Note saved", "notes": get_all_notes(user_id=user_id)}

@app.delete("/notes/{index}")
async def api_delete_note(index: int, user_id: str = Query(default="default")):
    ok = delete_note(index, user_id=user_id)
    if not ok:
        return {"error": f"No note at index {index}"}
    return {"message": "Deleted", "notes": get_all_notes(user_id=user_id)}
...
@app.get("/local-events")
async def list_local_events(user_id: str = Query(default="default")):
    events = get_all_events(user_id=user_id)
    return {
        "events": [{"index": i, **e} for i, e in enumerate(events)],
        "count": len(events),
...
@app.delete("/local-events/{index}")
async def api_delete_local_event(index: int, user_id: str = Query(default="default")):
    ok = delete_local_event(index, user_id=user_id)
    if not ok:
        return {"error": f"No event at index {index}"}
    return {"message": "Deleted", "events": get_all_events(user_id=user_id)}
...
@app.get("/memories")
async def list_memories(user_id: str = Query(default="default")):
    return {"memories": get_all_memories(user_id=user_id)}
...
async def api_save_memory(data: dict):
    ...
    if not fact:
        return {"error": "fact cannot be empty"}
    record = save_memory(fact, user_id=user_id)
    return {"message": "Memory saved", "memory": record}
...
async def api_search_memories(data: dict):
    ...
    if not query:
        return {"error": "query cannot be empty"}
    return {"results": search_memories(query, top_k=top_k, user_id=user_id)}

@app.delete("/memories/{memory_id}")
async def api_delete_memory(memory_id: str, user_id: str = Query(default="default")):
    deleted = delete_memory(memory_id, user_id=user_id)
    return {"message": f"Deleted {memory_id}"} if deleted else {"error": "Not found"}

@app.delete("/memories")
async def api_clear_memories(user_id: str = Query(default="default")):
...
async def pdf_chat(data: dict):
    try:
        ...
        chunks    = retrieve_relevant_chunks(question, namespace=namespace, top_k=5)
        if not chunks:
            return {"answer": "I couldn't find relevant content in this PDF. Try rephrasing your question.", "chunks": []}
        context_parts = []
        for i, chunk in enumerate(chunks):
            context_parts.append(f"[Chunk {i+1} | Page {chunk['page']} | Relevance {round(chunk['score']*100)}%]\n{chunk['text']}")
        context = "\n\n---\n\n".join(context_parts)
        prompt = (
            f"You are answering a question about a document.\n\n"
            f"CONTEXT:\n\n{context}\n\n"
            f"QUESTION: {question}\n\n"
            f"Answer using ONLY the context above. Cite page numbers when possible."
        )
        completion = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
        ...
        return {"answer": completion.choices[0].message.content, "chunks": chunks, "context_used": len(chunks)}
    except Exception as e:
        return {"error": str(e)}
...
@app.get("/dashboard")
async def dashboard(user_id: str = Query(default="default")):
    pdf_count = (
        len([f for f in os.listdir(UPLOADS_DIR) if f.endswith(".pdf")])
        if os.path.exists(UPLOADS_DIR) else 0
    )
    local_event_count = len(get_all_events(user_id=user_id))
    gcal_count = 0
    ...
    return {
        "todos":        len(get_all_todos(user_id=user_id)),
        "events":       local_event_count + gcal_count,
        "local_events": local_event_count,
        "gcal_events":  gcal_count,
        "notes":        len(get_all_notes(user_id=user_id)),
        "memories":     len(get_all_memories(user_id=user_id)),
        "pdfs":         pdf_count,
    }
...
async def google_callback(request: FastAPIRequest):
    ...
    try:
        ...
        return HTMLResponse(_error_page(str(exc)))
    if not user_id:
        return HTMLResponse(_error_page("State mismatch — please try connecting again."))
    return HTMLResponse(f"""
    <html><head><title>Connected!</title></head>
    <body style="font-family:sans-serif;background:#050510;color:#f0f0ff;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;">
      <div style="text-align:center;">
        <div style="font-size:48px;margin-bottom:12px;">✅</div>
        <h2 style="color:#34d399;margin:0 0 8px;">Google Calendar Connected!</h2>
        <p style="color:rgba(255,255,255,0.5);font-size:13px;">Signed in as <strong style="color:#a78bfa">{user_id}</strong></p>
        <p style="color:rgba(255,255,255,0.3);font-size:12px;">This window will close automatically…</p>
      </div>
      <script>
        if (window.opener) {{ window.opener.postMessage({{ type: 'GOOGLE_AUTH_SUCCESS', user_id: '{user_id}' }}, '*'); }}
        setTimeout(() => window.close(), 2000);
      </script>
    </body></html>
    """)
...
def _error_page(message: str) -> str:
    ...
    <body style="font-family:sans-serif;background:#050510;color:#f0f0ff;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;">
      <div style="text-align:center;max-width:420px;padding:20px;">
        <div style="font-size:48px;margin-bottom:12px;">❌</div>
        <h2 style="color:#f87171;margin:0 0 12px;">Connection Failed</h2>
        <p style="background:rgba(248,113,113,0.1);border:1px solid rgba(248,113,113,0.3);border-radius:10px;padding:12px 16px;font-size:13px;color:rgba(255,255,255,0.7);line-height:1.6;word-break:break-word;">{message}</p>
      </div>
      <script>
        if (window.opener) {{ window.opener.postMessage({{ type: 'GOOGLE_AUTH_ERROR', error: '{safe_msg}' }}, '*'); }}
        setTimeout(() => window.close(), 6000);
      </script>
    </body></html>
    """
...
async def google_status(user_id: str):
    connected = is_connected(user_id)
    info      = get_cached_user_info(user_id) if connected else {}
    return {"connected": connected, "email": info.get("email", ""), "name": info.get("name", ""), "picture": info.get("picture", "")}
...
async def calendar_create(data: dict):
    try:
        event = create_event(
            ...
            start_iso=data["start_iso"], end_iso=data["end_iso"],
            description=data.get("description", ""), location=data.get("location", ""),
            attendees=data.get("attendees", []), timezone_str=data.get("timezone", "UTC"),
        )
        return {"event": event, "message": f"Event created: {event['title']}"}
    except Exception as e:
        return {"error": str(e)}
...
{{"title":"event title","start_iso":"YYYY-MM-DDTHH:MM:00","end_iso":"YYYY-MM-DDTHH:MM:00","description":"","location":"","timezone":"UTC"}}
If duration not specified, assume 1 hour."""
        completion = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
        ...
        parsed = json.loads(raw)
        event  = create_event(
            user_id=user_id, title=parsed["title"],
            start_iso=parsed["start_iso"], end_iso=parsed["end_iso"],
            description=parsed.get("description", ""), location=parsed.get("location", ""),
            timezone_str=parsed.get("timezone", "UTC"),
        )
        return {"event": event, "message": f"✅ Created: {event['title']} on {event['start_human']}"}
    except Exception as e:
        return {"error": str(e)}
...
async def websocket_stt(websocket: WebSocket):
    ...
    async def on_message(self, result, **kwargs):
        try:
            ...
            completion = client.chat.completions.create(
                model="llama-3.3-70b-versatile",
                messages=[
                    {"role": "system", "content": "You are VoxMind Chronos, an AI second brain. Be helpful, concise, and conversational."},
                    {"role": "user", "content": transcript},
                ],
            )
            answer = completion.choices[0].message.content
            await websocket.send_json({"type": "ai_response", "transcript": transcript, "response": answer})
        except Exception as e:
            print(f"[STT] on_message error: {e}")
            try:
                await websocket.send_json({"type": "error", "message": str(e)})
    ...
    options = LiveOptions(model="nova-2", language="en-US", smart_format=True, interim_results=True, punctuate=True, endpointing=300)
    started = await dg_connection.start(options)
    if not started:
...
async def tts_speak(data: dict):
    try:
        ...
        return StreamingResponse(
            stream_tts(text, voice_id=voice_id),
            media_type="audio/mpeg",
            headers={"Cache-Control": "no-cache", "X-Content-Type-Options": "nosniff", "Access-Control-Allow-Origin": "*"},
        )
    except Exception as e:
        return {"error": str(e)}
...
async def tts_status():
    has_key = bool(os.getenv("ELEVENLABS_API_KEY"))
    return {"configured": has_key, "enabled": is_enabled(), "message": "ElevenLabs ready" if has_key else "Add ELEVENLABS_API_KEY to .env"}
    ...
    try:
        ...
        async def on_message(self, result, **kwargs):
            try:
                ...
                if is_final:
                    append_transcript(session_id, speaker, text, time_str)
                    await websocket.send_text(json.dumps({"type": "transcript", "speaker": speaker, "text": text, "is_final": True, "time": time_str}))
                else:
...
{{"summary":"...","action_items":[],"decisions":[],"conflicts":[],"key_topics":[],"sentiment":"positive|neutral|tense"}}"""
    try:
        completion = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
        ...
        data = {"summary": completion.choices[0].message.content, "action_items": [], "decisions": [], "conflicts": [], "key_topics": [], "sentiment": "neutral"}
    except Exception as e:
        return {"error": f"LLM analysis failed: {str(e)}"}
    return {**data, "session_id": session_id, "title": session.get("title", "Meeting"), "duration": duration, "speakers": speakers, "transcript": session["transcript"], "word_count": len(transcript_text.split())}

@app.get("/meeting/sessions")
async def meeting_sessions():
    from app.shadow_meeting import _sessions
    return {"sessions": [{"session_id": sid, "title": s.get("title", "Meeting"), "start": s.get("start"), "active": s.get("active", False), "lines": len(s.get("transcript", []))} for sid, s in _sessions.items()]}
...
async def meeting_transcript(session_id: str):
    ...
    if not session:
        return {"error": "Session not found"}
    return {"transcript": session["transcript"], "title": session.get("title", "Meeting"), "start": session.get("start")}