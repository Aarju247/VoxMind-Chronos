...
# ── Safe Lazy Initialization Configuration ───────────────────
_index_name = os.getenv("PINECONE_INDEX_NAME", "voxmind-memory")
_EMBED_MODEL  = "llama-text-embed-v2"
_CHUNK_SIZE   = 500    # characters per chunk  (~125 tokens)
_CHUNK_OVERLAP = 100   # overlap between consecutive chunks
...
def _get_pc_client():
    """Helper to safely construct the Pinecone client instance on demand."""
    api_key = os.getenv("PINECONE_API_KEY")
    if not api_key:
        raise ValueError("Critical Fault: PINECONE_API_KEY is missing from environment variables.")
    return Pinecone(api_key=api_key)


def _get_index():
    """Helper to safely extract the index connection context on demand."""
    return _get_pc_client().Index(_index_name)
...
def _embed_batch(texts: List[str], input_type: str = "passage") -> List[List[float]]:
    ...
    for i in range(0, len(texts), BATCH):
        ...
        # Safe extraction layer fallback matching variant versions of the Pinecone SDK
        try:
